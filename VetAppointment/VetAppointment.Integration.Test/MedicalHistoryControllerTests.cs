...
//    {
//        private const string ApiURL = "v1/api/medicalHistories";
...
//            var clinicDto = CreateClinicSUT();
//            var clinicResponse = await HttpClient.PostAsJsonAsync("v1/api/vetclinics", clinicDto);
//            var clinic = clinicResponse.Content.ReadFromJsonAsync<VetClinicDto>().Result;

//            var appointmentSut = CreateAppoinmentSUT();

//            // Act
//            var response = await HttpClient.PostAsJsonAsync($"{ApiURL}/{clinic.MedicalHistoryId}", appointmentSut);
